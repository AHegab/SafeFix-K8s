...
.DESCRIPTION
  ...
  Produces:
    - validation/reports/<file-id>.json   (per-file gate results)
    - validation/evidence/**              (raw outputs per gate)
    - validation/safe_fix_proof.json      (aggregate signed proof)

.PARAMETER InputDir
  Directory containing YAML/YML manifests to validate. Defaults to output/patch_sandbox.

.PARAMETER OutputDir
  Directory to write reports/evidence (default: validation).
...
.EXAMPLE
  ...
  .\validation\validate-gates.ps1 -InputDir .\output\patch_sandbox -Gates schema,policy,dryrun

.EXAMPLE
  # Full run against current kube-context (if available):
  .\validation\validate-gates.ps1 -InputDir .\output\patch_sandbox -EnableSandbox -TimeoutSec 90

#>
[CmdletBinding()] param(
  [Parameter(Position=0)] [string] $InputDir = "output/patch_sandbox",
  [Parameter()] [string] $OutputDir = "validation",
...
function Gate-Policy([string]$file, [string]$evidDir) {
  ...
  $pol = Join-Path $root 'policies/opa'
...
function Gate-Sandbox([string]$file, [string]$evidDir, [string]$ns) {
  ...
  try {
    kubectl get ns $ns *>$null 2>&1 || kubectl create ns $ns *>$null 2>&1
  } catch {}
...
function Gate-E2E([string]$evidDir, [string]$ns) {
  # Stub: look for validation/e2e_smoke.ps1 and execute if present
  New-Dir $evidDir
...
if ($EnableSandbox -and $hasKubectl) {
  try { kubectl get ns $Namespace *>$null 2>&1 || kubectl create ns $Namespace *>$null 2>&1 } catch {}
}
...
# Aggregate proof
$summary = @{ PASS = ($aggregate | Where-Object { $_.overall -eq 'PASS' }).Count
              FAIL = ($aggregate | Where-Object { $_.overall -eq 'FAIL' }).Count }
