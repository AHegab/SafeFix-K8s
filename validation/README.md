...
Artifacts:
- `validation/reports/<file-id>.json` per file
- `validation/evidence/**` gate-specific raw outputs
...
```powershell
.\validation\validate-gates.ps1 -InputDir .\output\patch_sandbox -EnableSandbox -SigningKey (Get-Content .\.signing_key)
```

Optional E2E hook: create `validation/e2e_smoke.ps1` and return non-error for PASS.
...
You can then review per-file reports in `validation/reports/` and the aggregate proof at `validation/safe_fix_proof.json`.
